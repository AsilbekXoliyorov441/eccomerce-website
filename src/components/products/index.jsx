...
const ProductCard = ({ name, url, discount, rating, article, price }) => {
    return (
        <div className="hover:shadow-xl cursor-pointer  bg-[white] group duration-300 max-w-[200px] sm:max-w-[350px] w-full border-[1px] rounded-[6px] overflow-hidden p-[7px] sm:p-[20px]  border-[#E3E3E8]">
            <img className='' src={url} alt="Img" />
            <div className='flex items-center justify-between gap-[10px] pt-[14px]'>
                <p className='text-[#7A7680] text-[9px] sm:text-[12px] xl:text-[13px] whitespace-nowrap'>{article}</p>
                <img className='max-w-[67px] sm:max-w-[117px]' src="/rating.png" alt="Star" />
            </div>
            <div className='pt-[5px] sm:pt-[10px] line-clamp-2 text-[13px] sm:text-[15px] xl:text-[16px]'>
                <h3>{name}</h3>
            </div>
            <div className='flex items-center justify-between gap-[10px] pt-[6px] sm:pt-[16px]'>
                <div className='flex items-start gap-[10px]'>
                    <p className='text-[16px] sm:text-[18px] sm:text-[21px] whitespace-nowrap xl:text-[30px] '>{price}</p>
                    <strike className="text-[#7A7680] whitespace-nowrap text-[10px] sm:text-[14px] xl:text-[16px]">{discount}</strike>
                </div>
                <Link to={"cart"} className='border-[2px] cursor-pointer group hover:fill="#ffff" border-[#5946D7] p-[5px] rounded-[8px] bg-transparent hover:bg-[#5946D7] duration-200'>
